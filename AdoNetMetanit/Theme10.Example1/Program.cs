...
class Program
{
    static void Main(string[] args)
    {
        ...
        var npsqlExpression = @"INSERT INTO clients (status, fullname, clientage, contactdetails, isblocked, isanonymous) 
                                values (@status, @fullName , @age, @contact, @isBlocked, @isAnonymous) returning id";

        var status = ClientStatus.STANDART;
        var fullName = "'Ивкин Сергей Викторович'";
        var age = 59;
        var contact = @"'ivkin_sport@mail.ru'";
        var isBlocked = false;
        ...
        using (var connection = new NpgsqlConnection(connectionString))
        {
            ...
            Console.WriteLine("Id : {0}", idParam.Value);
        }
    }
}